/*
    * DDL ( DATE DEFINITON LANGUAGE) : 데이터 정의 언어
...
SELECT * FROM MEMBER;
-- 사용자가 가지고 있는 테이블 정보
-- 데이터 딕셔너리(사전) : 다양한 객체들의 정보를 저장하고 있다.
...
-------------------------------------------------------------------------------
 /*
    3. 제약조건 (CONSTRAINTS)
      ...
      - 데이터 무결성 보장을 목적으로 한다
        : 데이터의 결함이 없는 상태, 즉 데이터가 정확하고 유효하게 유지된 상태이다.
        
        1. 개체 무결성 제약 조건 : NOT NULL, UNIQUE(겹치면안댐!! EX) 주민등록번호) , PRIMARY KEY 조건 위배 
...
/*
       ...
       2) 테이블 레벨 방식
       : 모든 컬럼들을 나열한 후 마지막에 기술
  */
  
  /*
  
     * NOT NULL 제약조건
     : 해당 컬럼에 반드시 값이 존재해야만 할 경우
       (즉, 컬럼 값에 NULL이 들어오면 안된다는 소리)
...
-- 컬럼 레벨 방식 : NOT NULL 제약 조건은 컬럼 방식만 사용가능
...
INSERT INTO MEM_NOTNULL VALUES(1,'user01', 'pass01', '이고잉', '여', '010-1234-5678', 'user01@gmail.com');
INSERT INTO MEM_NOTNULL VALUES(1,'user01', null, '이고잉', '여', null, 'user01@gmail.com'); -- 오류 : 비밀번호의 null값 허용 안함
-- NOT NULL 제약 조건에 위배되는 오류
...
INSERT INTO MEM_UNIQUE VALUES(2, 'user01', 'pass01', '채규태', '여', '010-1234-5678', 'user01@gmail.com'); -- 오류
-- 오류 unique 제약 조건 위배
...
INSERT INTO MEM_UNIQUE2 VALUES(2, 'user01', 'pass01', '채규태', '여', '010-1234-5678', 'user01@gmail.com'); -- 오류
-- 오류 unique 제약 조건 위배


-- 테이블 레벨 방식
-- 2개의 컬럼을 묶어서 중복 값을 확인 ( ex : (1,user01) != (1, user02) )
...
INSERT INTO MEM_UNIQUE4 VALUES(2, 'user01', 'pass01', '채규태', '여', '010-1234-5678', 'user01@gmail.com'); --오류 아님!
    -- UNIQUE (MEM_NO, MEM_ID) 안쓰는 이유는 "한쌍" 으로 보기 때문이다.
    -- (1, 'user01') 이랑 (2, 'user02') 은 다른거임 () 을 한쌍으로 보기때문에 1과 2은 다른 것이므로 () 은 다른 거임!!
INSERT INTO MEM_UNIQUE4 VALUES(3, 'user03', 'pass03', '우재남', 'ㄴ', '010-1234-5678', 'user01@gmail.com'); -- 'ㄴ' 으로해도 오류는 안뜬다.
-- > 성별이 유효한 값이 아니어도 오류가 안뜬다 그럼 어떻게해야할까~?


--------------------------------------------------------------------------------
/*
    CHECK(조건식) 제약조건
    : 사용자가 정의 제약 조건을 넣고 싶을 때
*/
...
CREATE TABLE MEM_CHECK(
    ...
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CHECK(GENDER IN('남','여')), -- 남, 여를 제외하고는 다 오류가 뜨게 조건을 둠
    PHONE VARCHAR(13),
    EMAIL VARCHAR2(50)
    );
    
INSERT INTO MEM_CHECK VALUES(1, 'user01', 'pass01', '이고잉', '여', '010-1234-5678', 'user01@gmail.com');
INSERT INTO MEM_CHECK VALUES(2, 'user02', 'pass02', '우재남', 'ㄴ', '010-1234-5678', 'user01@gmail.com'); --오류
-- CHECK 제약조건에 위배
...
/*
   * PRIMARY KEY (기본키) 제약조건
   : 테이블에서 각 행들을 식별하기 위해 사용되는 컬럼에 부여하는 제약조건
     (식별자의 역할)
     
     EX) 회원번호, 학번, 사원번호, 주문번호, 예약번호, 운송장 번호, ...
     
     - PRIMARY KEY(기본키) 제약조건을 부여하면 NOT NULL + UNIQUE 제약 조건을 의미
     >> 대체적으로 검색, 수정, 삭제할 때 기본키의 컬럼값을 이용한다.
...
INSERT INTO MEM_PRIMARY VALUES (2, 'user03', 'pass03', '이고잉', '여', null, null); -- 오류
-- 제약 조건 Primary_Key 때문에 '2' 중복이라서 못함!
...
*/
-- 테이블 레벨 방식
-- 제약조건 이름 변경 : CONSTRAINT + 제약조건의이름 + 제약조건식
...
CREATE TABLE MEM_PRIMARY3(
   MEM_NO NUMBER NOT NULL ,
    ...
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CONSTRAINT MEM_GENDER CHECK(GENDER IN('남','여')), --  CONSTRAINT + 제약조건의이름 : MEM_GENDER + 제약조건식 : CHECK(GENDER IN('남','여'))
    PHONE VARCHAR(13),
    EMAIL VARCHAR2(50),
    CONSTRAINT MEM_PK PRIMARY KEY(MEM_NO) -- CONSTRAINT + 제약조건의이름 : MEM_PK + 제약조건식 : PRIMARY KEY(MEM_NO)
    );
...
-- 복합 키
-- 기본 키를 2개 넣었을 때 2개를 묶어서 하나의 기본키로 설정됨
-- 테이블 레벨 방식으로만 가능하다
CREATE TABLE MEM_PRIMARY4(
   MEM_NO NUMBER NOT NULL ,
    MEM_ID VARCHAR2(20) NOT NULL CONSTRAINT ID_UNIQUE UNIQUE, -- CONSTRAINT + 제약조건의이름 : ID_UNIQUE + 제약조건식 : UNIQUE
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CONSTRAINT MEM_GENDER CHECK(GENDER IN('남','여')), --  CONSTRAINT + 제약조건의이름 : MEM_GENDER + 제약조건식 : CHECK(GENDER IN('남','여'))
    PHONE VARCHAR(13),
    EMAIL VARCHAR2(50),
    CONSTRAINT PK PRIMARY KEY(MEM_NO, MEM_ID) -- CONSTRAINT + 제약조건의이름 : PK + 제약조건식 : PRIMARY KEY(MEM_NO, MEM_ID) < 복합 키>
      -- PRIMARY KEY(MEM_NO, MEM_ID)는 UNIQUE (MEM_NO, MEM_ID) 와 똑같이 "한쌍" 으로 본다!
...
-- 복합키 제약조건 오류
INSERT INTO TB_LIKE VALUES(1, 'A', SYSDATE); -- 오류 ( 또 찜 하면 안대!!)
INSERT INTO TB_LIKE VALUES(2, 'B', SYSDATE); -- 오류 ( 또 찜 하면 안대!!)

-- 복학키도 기본키이기 때문에 NOT NULL + UNIQUE 다 맞아야 된다!
-- 각 컬럼은 NULL값이 안되고, 2개의 컬럼을 합쳐서 유일해야 된다.
INSERT INTO TB_LIKE VALUES(NULL, 'A', SYSDATE); -- 오류 ( NULL 값 삽입 불가능 )
...
INSERT INTO MEM VALUES(3, 'user03', 'pass03', '채규태', '남', 100); -- 오류떠야하는데 왜안뜰까?
-- 유효한 회원등급 번호가 아님에도 불구하고 입력된다.
...
/*
    ...
    >>컬럼 레벨 방식
      컬럼명 자료형 [CONSTRAINT 제약 조건명]REFERENCES 참조할테이블명[(참조할컬럼명)]
...
CREATE TABLE MEM2(
    ...
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CHECK(GENDER IN('남','여')),
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GRADE_CODE) -- NOT NULL을 넣으면 NULL값 허용 안댄다는 소리 // --컬럼 레벨 방식
...
-- 외래키 제약조건은 기본적으로 NULL값을 허용한다.

INSERT INTO MEM2 VALUES(3, 'user03', 'pass03', '채규태', '남', 100); -- 오류 : 무결성 제약조건(DDL.SYS_C007436)이 위배되었습니다- 부모 키가 없습니다. 가 뜬다. 여기서 부모는 누구일까?
-- 부모는 MEM테이블이다!
-- 다라서 MEM2는 자식 테이블이다!
-- 외래키 제약조건 위배
...
-- 자식테이블에서 사용하고 있는 컬럼값은 삭제가 불가능
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10; -- 오류
-- 자식 레코드가 발견되었습니다 << 자식이 있으니 삭제하면 안댄다라는 소리
...
/*
    ...
    * 자식테이블 생성시 외래키 제약조건 부여할 때 삭제옵션 지정 가능
    - 삭제 옵션 : 부모테이블의 데이터 삭제 시 자식 테이블이 사용하고 있는 값을 어떻게 처리할지
    
    1) ON DELETE RESTRICTED(기본 값) : 삭제 제한 옵션으로, 자식테이블이 쓰고 있는 값이면 부모테이블에서 삭제 안됨
...
--외래키 생성시 참조 테이블명만 넣으면 참조테이블의 기본키의 컬럼이 자동으로 설정됨
CREATE TABLE MEM(
    MEM_NO NUMBER CONSTRAINT PK PRIMARY KEY,
    MEM_ID VARCHAR2(20) NOT NULL UNIQUE,
    MEM_PWD VARCHAR2(20) NOT NULL,
...
DELETE FROM MEM_GRADE WHERE GRADE_CODE = 30; -- 이거하면 부모 테이블 30 없어지고, 자식테이블 30도 없어짐


-------------------------------------------------------------------------------
/*
    * DEFAULT 값 설정하기
    컬럼의 값이 들어오지 않았을 때 기본 값으로 넣어준다.
...
-- 순서 똑같이 값을 넣어줘야한다 ! -> MEM_NO : 4 ,  MEM_ID : user04 , MEM_AGE : 25
...
-- EMPLOYEE테이블을 복제(컬럼값 제외 구조만)한 새로운 테이블 생성
CREATE TABLE EMPLOYEE_COPY2
AS SELECT *
FROM EMPLOYEE
WHERE 1=0; -- 여기서말하는 1과 0은 참과 거짓을 말하는 것이다.
-- 구조만 가져올때 이 WHERE절을 쓴다고 생각하기! (데이터는 없음)

-- EMPLOYEE테이블을 복제(컬럼값 제외 구조만)한 컬럼을 내가 원하는 컬럼만으로 새로운 테이블 생성
...
-- EMPLOYEE테이블을 복제(컬럼값 제외 구조만)한 컬럼을 기준테이블에 없는 컬럼도 생성
CREATE TABLE EMPLOYEE_COPY4
AS SELECT EMP_NO, EMP_NAME, SALARY, SALARY*12 연봉
FROM EMPLOYEE;
--SALARY*12을 넣은 때 별창도 함께 지정 해야 오류가 안뜬다!
-- 서브쿼리 SELECT절에 산술식 또는 함수식이 기술된 경우 반드시 별칭을 지정해야한다
...
-- 삭제하고 다시만든건 쌤이 ID 안넣고해서 그런겨 ㅇㅇ 집에가서할때 왜했지 씨부리지말기 ㅇㅇ
...
INSERT INTO TB_PUBLISHER VALUES(1, 'user01', '뀨');
INSERT INTO TB_PUBLISHER VALUES(2, 'user02', '뀨우');
INSERT INTO TB_PUBLISHER VALUES(3, 'user03', '뀨우우');
...
CREATE TABLE TB_BOOK(
     ...
     BK_PRICE NUMBER,
     PUB_NO NUMBER REFERENCES TB_PUBLISHER(PUB_NO) ON DELETE CASCADE
);
...
COMMENT ON COLUMN TB_BOOK.PUB_NO IS '출판사번호';

INSERT INTO TB_BOOK VALUES(1, 'user01', '뀨', 100, 1);
INSERT INTO TB_BOOK VALUES(2, 'user02', '뀨우', 200, 2);
INSERT INTO TB_BOOK VALUES(3, 'user03', '뀨우우', 300, 3);
INSERT INTO TB_BOOK VALUES(4, 'user04', '뀨우우우', 300, 3);
INSERT INTO TB_BOOK VALUES(5, 'user05', '뀨우우우우', 250, 1);
INSERT INTO TB_BOOK VALUES(6, 'user06', '뀨우우우우', 550, 2);


/*
3. 회원에 대한 데이터를 담기위한 회원 테이블 (TB_MEMBER)
   컬럼명 : MEMBER_NO(회원번호) NUMBER -- 기본키(MEMBER_PK) ㅇ
   MEMBER_ID(아이디) VARCHAR2(30) -- 중복금지(MEMBER_UQ) ㅇ
...
INSERT INTO TB_MEMBER VALUES(1, 'user01', 'pwd01', 'A', 'F', 'a', '1a', 'Y', DEFAULT);
INSERT INTO TB_MEMBER VALUES(2, 'user02', 'pwd02', 'B', 'F', 'b', '2b', 'Y', '2024-06-10');
INSERT INTO TB_MEMBER VALUES(3, 'user03', 'pwd03', 'C', 'F', 'c', '3c', 'N', '2024-06-12');
INSERT INTO TB_MEMBER VALUES(4, 'user04', 'pwd04', 'D', 'M', 'd', '4d', 'Y', '2024-06-04');
INSERT INTO TB_MEMBER VALUES(5, 'user05', 'pwd05', 'E', 'M', 'e', '5e', 'N', DEFAULT);
INSERT INTO TB_MEMBER VALUES(6, 'user06', 'pwd06', 'F', 'F', 'f', '6f', 'Y', '2024-06-01');
INSERT INTO TB_MEMBER VALUES(7, 'user07', 'pwd07', 'G', 'M', 'g', '7g', 'Y', '2024-06-15');
INSERT INTO TB_MEMBER VALUES(8, 'user08', 'pwd08', 'H', 'F', 'h', '8h', 'N', DEFAULT);
INSERT INTO TB_MEMBER VALUES(9, 'user09', 'pwd09', 'I', 'M', 'i', '9i', 'N', DEFAULT);
...
CREATE TABLE TB_RENT(
    ...
    TB_MEMBER NUMBER REFERENCES TB_MEMBER(MEMBER_NO) ON DELETE CASCADE,
    TB_BOOK NUMBER REFERENCES TB_BOOK(BK_NO) ON DELETE CASCADE,
    RENT_DATE DATE DEFAULT sysdate
);

COMMENT ON COLUMN TB_RENT.RENT_NO IS '대여번호';
COMMENT ON COLUMN TB_RENT.TB_MEMBER IS '대여회원번호';
COMMENT ON COLUMN TB_RENT.TB_BOOK IS '대여도서번호'; 
COMMENT ON COLUMN TB_RENT.RENT_DATE IS '대여일';

INSERT INTO TB_RENT VALUES(1, 8 ,1 , null);
INSERT INTO TB_RENT VALUES(2, 7 ,3 , '2024-06-12');
INSERT INTO TB_RENT VALUES(3, 9 ,6 , null);
INSERT INTO TB_RENT VALUES(3, 9 , , null);
